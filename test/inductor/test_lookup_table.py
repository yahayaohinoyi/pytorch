...
class TestLookupTable(TestCase):
    ...
    def test_lookup_key_generation(self):
        ...
        with patch(
            "torch._inductor.lookup_table.get_size_hint"
        ) as mock_size_hint, patch(
            "torch._inductor.lookup_table.get_stride_hint"
        ) as mock_stride_hint:
...
class TestLookupTableFileParsing(TestCase):
    ...
    def test_json_file_parsing(self):
        ...
        try:
            ...
            self.assertEqual(
                torch._inductor.lookup_table.gemm_config_lookup_table, test_data
            )
        finally:
